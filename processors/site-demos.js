...
async function generateSiteDemos() {
    try {
        ...
        for (const widgetFolder of widgetFolders) {
            ...
            // don't build getting started demos
            if (widgetFolderPath.indexOf('getting-started') !== -1) {
                continue;
            }

            if (!await isDirectory(widgetFolderPath)) {
                continue;
            }

            // creates the widget folder in release/demos
            await createFolder(widgetFolder);

            // get the demos folders
            const demosFolders = await readDir(widgetFolderPath);

            for (const demoFolder of demosFolders) {
                const demoFolderPath = joinPaths(widgetFolderPath, demoFolder);

                if (!await isDirectory(demoFolderPath)) {
                    continue;
                }
...
async function createFolder(...paths) {
    const destionation = joinPaths('..', 'release', 'demos', ...paths)
...
async function removeFilesInDevProject() {
    const bin = joinPaths('..', 'dev-project', 'bin');
    const obj = joinPaths('..', 'dev-project', 'obj');
    const appRazor = joinPaths('..', 'dev-project', 'App.razor');
    const indexJs = joinPaths('..', 'dev-project', 'wwwroot', 'index.js');

    if (fs.existsSync(bin)) {
        await fsPromises.rmdir(bin, { recursive: true });
    }

    if (fs.existsSync(obj)) {
        await fsPromises.rmdir(obj, { recursive: true });
    }

    if (fs.existsSync(appRazor)) {
        await fsPromises.unlink(appRazor);
    }

    if (fs.existsSync(indexJs)) {
        await fsPromises.unlink(indexJs);
    }

    return;
}

async function copyDemoFilesToDevProject(demoFolderPath, demoFiles) {
    for (const file of demoFiles) {
        const filePath = joinPaths(demoFolderPath, file);
        let destinationPath = '';

        switch (filePath.indexOf('razor')) {
            case -1:
                destinationPath = joinPaths('..', 'dev-project', 'wwwroot', 'index.js');
                break;
            default:
                destinationPath = joinPaths('..', 'dev-project', 'App.razor');
        }
...
async function buildDevProject() {
    return new Promise(resolve => {
        const cwd = joinPaths('..', 'dev-project');
...
async function copyBuildFilesToRelease(...paths) {
    ...
    const destination = joinPaths('..', 'release', 'demos', ...paths);
    const indexJsToPath = joinPaths(destination, 'index.js');
    const indexDllToPath = joinPaths(destination, 'index.dll');

    if (fs.existsSync(indexJsFromPath)) {
        await fsPromises.copyFile(indexJsFromPath, indexJsToPath);
    } else {
        await fsPromises.writeFile(indexJsToPath, '');
    }

    await fsPromises.copyFile(indexDllFromPath, indexDllToPath);

    return;
}

async function createCommonDemoFiles(...paths) {
    const destination = joinPaths('..', 'release', 'demos', ...paths);
    const blazorBootPath = joinPaths(destination, 'blazor.boot.json');
    const blazorWebAssemblyPath = joinPaths(destination, 'blazor.webassembly.js');

    const blazorBootData = getBlazorBootData(...paths);
    const blazorWebAssemblyData = getBlazorWebAssemblyData(...paths);

    await fsPromises.writeFile(blazorBootPath, blazorBootData);
    await fsPromises.writeFile(blazorWebAssemblyPath, blazorWebAssemblyData);
}

function getBlazorBootData(...paths) {
...
`{
    "assemblies": 
    [
        "../framework/_framework/_bin/Microsoft.AspNetCore.Blazor.dll",
        "../framework/_framework/_bin/Microsoft.AspNetCore.Blazor.HttpClient.dll",
        "../framework/_framework/_bin/Microsoft.AspNetCore.Components.Web.dll",
        "../framework/_framework/_bin/Microsoft.AspNetCore.Components.dll",
        "../framework/_framework/_bin/Microsoft.Bcl.AsyncInterfaces.dll",
        "../framework/_framework/_bin/Microsoft.Extensions.Configuration.Abstractions.dll",
        "../framework/_framework/_bin/Microsoft.Extensions.Configuration.dll",
        "../framework/_framework/_bin/Microsoft.Extensions.DependencyInjection.Abstractions.dll",
        "../framework/_framework/_bin/Microsoft.Extensions.DependencyInjection.dll",
        "../framework/_framework/_bin/Microsoft.Extensions.Logging.Abstractions.dll",
        "../framework/_framework/_bin/Microsoft.Extensions.Primitives.dll",
        "../framework/_framework/_bin/Microsoft.JSInterop.dll",
        "../framework/_framework/_bin/Mono.WebAssembly.Interop.dll",
        "../framework/_framework/_bin/System.Core.dll",
        "../framework/_framework/_bin/System.Net.Http.dll",
        "../framework/_framework/_bin/System.Runtime.CompilerServices.Unsafe.dll",
        "../framework/_framework/_bin/System.Text.Encodings.Web.dll",
        "../framework/_framework/_bin/System.Text.Json.dll",
        "../framework/_framework/_bin/System.dll",
        "../framework/_framework/_bin/WebAssembly.Bindings.dll",
        "../framework/_framework/_bin/WebAssembly.Net.Http.dll",
        "../framework/_framework/_bin/jQWidgets.Blazor.dll",
        "../framework/_framework/_bin/mscorlib.dll",
        "../demos/${path}/index.dll"
    ],
    "entryAssembly": "index",
    "linkerEnabled": true
}    
